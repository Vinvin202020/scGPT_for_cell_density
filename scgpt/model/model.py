...
class TransformerModel(nn.Module):
    def __init__(
        ...
        self.cdp_decoder = RegressionDecoder(d_model, nlayers=nlayers)
        if do_mvc:
    ...
    def forward(
        ...
        if CDP:
            output['CDP_output'] = self.cdp_decoder(cell_emb)
        if CCE:
            cell1 = cell_emb
...
class RegressionDecoder(nn.Module):
    ...
    def __init__(
        ...
        self.out_layer = nn.Linear(d_model, 1)  # Single output for regression
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        for layer in self._decoder:
            x = layer(x)
        return self.out_layer(x)  # No activation on the output
